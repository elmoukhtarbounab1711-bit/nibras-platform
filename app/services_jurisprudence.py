...
def _category_slug_to_id(conn, slug: str):
...
def list_decisions(category_slug=None, limit: int = DEFAULT_LIST_LIMIT, offset: int = 0):
...
def jurisprudence_stats():
    ...
    with db_session() as conn:
        ...
        q = "SELECT COUNT(*) FROM jurisprudence j WHERE j.published = 1"
        if dec_cond:
            q += " AND " + dec_cond
...
def create_decision(admin_id: int, data: dict) -> dict:
    ...
    with db_session() as conn:
        ...
        category_id = None
        if category_slug:
            category_id = _category_slug_to_id(conn, category_slug)
...
def update_decision(admin_id: int, decision_id: int, data: dict) -> dict:
    ...
    with db_session() as conn:
        row = conn.execute(
            "SELECT id FROM jurisprudence WHERE id = ?", (decision_id,)
        ).fetchone()
        if row is None:
        ...
        if "category_slug" in data:
            ...
            if slug:
                cat_id = _category_slug_to_id(conn, slug)
                if cat_id is None:
                    raise JurisprudenceError("الفئة غير موجودة.", 400)
...
def create_category(admin_id: int, data: dict) -> dict:
    ...
    with db_session() as conn:
        if _category_slug_to_id(conn, slug) is not None:
            raise JurisprudenceError("يوجد فئة بهذا slug بالفعل.", 409)
        cur = conn.execute(
            "INSERT INTO jurisprudence_categories (slug, name, description, tenant_id) "
            "VALUES (?, ?, ?, ?)",
