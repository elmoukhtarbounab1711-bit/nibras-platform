...
def _build_prompt(question: str, context_articles: list, mode: str,
    ...
    if mode == "general":
        system = (
            ...
            "═══ قاعدة أساسية: ZERO AI REWRITING ═══\n"
            "إذا أوردت نصًا قانونيًا رسميًا، فاعرضه كما هو من المصدر الرسمي.\n"
            "لا تعيد صياغة أي نص قانوني رسمي.\n"
            "يمكنك شرح النص أو تلخيصه بشكل عام، لكن بيّن أن ذلك شرح وليس النص الأصلي.\n\n"
        ...
        system = (
            ...
            "القانونية — أكثر من 4000 نص قانوني مغربي (74000+ مادة) واجتهادات "
            ...
            "═══ قاعدة أساسية: ZERO AI REWRITING ═══\n"
            "النصوص القانونية المرفقة مستخرجة من مصادرها الرسمية (عدالة، الأمانة العامة للحكومة، "
            "الجريدة الرسمية). يُمنع عليك تمامًا:\n"
            "- إعادة كتابة أي نص قانوني أو تعديله أو صياغته بطرق مختلفة\n"
...
def _build_research_prompt(question: str, context_articles: list, web_results: list,
    ...
    system = (
        ...
        "تعتمد مادة مادة من مكتبة نبراس القانونية (أكثر من 4000 نص قانوني مغربي "
        "محدث — 74000+ مادة) واجتهاداتها القضائية باعتبارها المرجع الملزم، "
        "وتقابلها بالمقالات الخارجية المسترجعة من الويب. التزم بالآتي:\n"
        "═══ قاعدة أساسية: ZERO AI REWRITING ═══\n"
        "النصوص القانونية من نبراس مستخرجة من مصادرها الرسمية (عدالة، الأمانة العامة للحكومة، "
        "الجريدة الرسمية). يُمنع عليك تمامًا إعادة كتابة أي نص قانوني أو تعديله.\n"
        "اعرض المواد القانونية من نبراس كما هي — لا تغيّر عليه.\n\n"
