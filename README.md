...
| `NIBRAS_MULTI_TENANT` | `0` | تفعيل عزل multi-tenant: عند `1` يُلزَم رأس `X-Tenant-Id` في كل الطلبات المصادَق عليها (403 للغياب/الجهل/التعليق) وتُصفَّى بيانات الوحدات بمستأجر الطلب؛ الافتراضي `0` يتجاهل الرأس ويبقي سلوك المستأجر الواحد. |
| `NIBRAS_DEFAULT_TENANT_SLUG` | `nibras` | معرّف المستأجر الافتراضي المبذور عند الإقلاع (منه يُلحق الترحيب القدامى). |
...
لوحة إدارة شاملة داخلية مستقلة (Admin Panel Spec §5) تتصل بالـ API نفسها
وتحمل عنوانها في `API_BASE`. تتيح تسجيل الدخول بحساب مسؤول ثم:

- **الرئيسية (Dashboard)**: ملخص الاستخدام — عدد المستخدمون/النصوص/المواد/
  المقالات/الحملات/القوالب/المستأجرون/طلبات التحقق.
- **إدارة المحتوى**: عرض/إنشاء/تعديل/حذف النصوص القانونية وموادها.
- **طابور التحقق**: عرض/قبول/رفض طلبات الأدوار المهنية مع سبب إلزامي.
- **الإشراف**: بلاغات المحتوى (إغلاق/إخفاء/إزالة) مسجّلة تدقيقًا.
- **المقالات**: إدارة مقالات بوابة المقالات وحالتها + بلاغات المقالات.
- **الإعلانات**: إدارة الحملات (تفعيل/إيقاف/حذف) + عرض الفتحات والإحصائيات.
- **السوق**: إدارة قوالب السوق وفئاتها.
- **المستأجرون**: قائمة المستأجرين.
- **القانون المقارن**: الولايات القضائية + إدارة حالات الدراسات (نشر/إخفاء).
- **الإشعارات**: إحصائيات التسليم + تفريغ صندوق الإشعارات.
...
## النشر (PaaS — Render / Railway / Fly)

الواجهة الأمامية تُخدم من نفس الخادم (Flask يقدّم `index.html` و`admin.html` و`/assets`) — لا حاجة لاستضافة منفصلة. التشغيل الإنتاجي عبر **gunicorn** (`Procfile`) على منفذ `$PORT`.

### التحضير (مرة واحدة)

1. أنشئ مستودع Git وادفع الكود (بما فيه `frontend/` — بدون `nibras.db` ولا `uploads/`):
   ```bash
   git init && git add -A && git commit -m "نبراس جاهزة للنشر"
   ```
2. جهّز **قاعدة البيانات** على قرص مُثبَّت. PaaS لها نظام ملفات عابر — أي ملف يُكتب خارج القرص المُثبَّت يُفقد عند كل إعادة نشر:
   - أنشئ Disk (Render) أو Volume (Railway/Fly) واربطه (مثلًا `mountPath: /data`).
   - انسخ `nibras.db` إلى القرص عبر **نسخة احتياطية** (موصى به): على جهازك `python scripts/backup.py backup` ثم ارفع ملف `.sqlite` الناتج إلى القرص، أو ضعه في مسار يقرؤه `copy` (انظر أدناه).
   - على قاعدة فارغة يكفي أمر `releaseCommand` الذي يشغّل `scripts/paas_bootstrap.py ensure` فيبذر مخططًا سليمًا تلقائيًا.

### متغيرات البيئة الإلزامية للإنتاج

| المتغير | الإلزامية | القيمة المقترحة |
|---|---|---|
| `NIBRAS_JWT_SECRET` | **إلزامي** | سر عشوائي ثابت طويل — بدونه تنتهي الجلسات عند كل إقلاع |
| `NIBRAS_DB_PATH` | إلزامي | `/data/nibras.db` (على القرص المُثبَّت) |
| `NIBRAS_UPLOAD_DIR` | مُوصى به | `/data/uploads` (وثائق التحقق وقوالب السوق) |
| `NIBRAS_CORS_ORIGINS` | مُوصى به | نطاقات الواجهة الفعلية، أو `*` لأول نشر |
| `NIBRAS_FRONTEND_BASE_URL` | إن أُرسل بريد | رابط الواجهة لروابط استعادة كلمة المرور |
| `NIBRAS_AI_PROVIDER` | اختياري | `noop` افتراضيًا؛ المزوّدات الحقيقية (Gemini/Groq...) تُضاف من لوحة التحكم بعد الإقلاع |

> كل الخيارات الأخرى في `.env.example` (نموذج يوثّق المعنى والقيم المقترحة).

### Render (Blueprint جاهز)

الملف `render.yaml` جاهز: خدمة Python + فحص صحة `/api/ready` + أمر إصدار يشغّل البوتستراب + `gunicorn`. الخطوات:

1. ادفع المستودع إلى GitHub.
2. Render → **New → Blueprint** → اختر المستودع (يقرأ `render.yaml` تلقائيًا).
3. فعّل Disk للخطة (المجاني لا يتضمن أقراصًا دائمة) أو استخدم خطة مدفوعة، ثم:
   - علّق على سطر `disk:` داخل `render.yaml` بإزالة الـ `#`، أو أنشئ القرص من لوحة Render وضبط `NIBRAS_DB_PATH`.
4. انسخ `nibras.db` إلى القرص (عبر نسخة احتياطية، راجع التحضير).

### Railway / Fly

- **Railway**: أنشئ مشروعًا → Deploy من GitHub → أضف **Volume** واربطه بـ `/data` → عيّن المتغيرات أعلاه → استبدل `startCommand` بقراءة `Procfile` (Railway يشغّله تلقائيًا عند وجوده).
- **Fly.io**: `fly launch` ثم `fly volumes create` واربطه، واعتمد `Procfile` (أو `fly deploy` مع متغيرات البيئة). اضبط `NIBRAS_DB_PATH` على مسار الحجم.

### استنساخ قاعدة قائمة إلى القرص (اختياري)

عند بدء قرص مُثبَّت يحتوي قاعدة محفوظة في مسار آخر (مثل نسخة مرفوعة مؤقتًا):
...
- `seed` — بذر مخطط فارغ سليم (لا يمسّ قاعدة قائمة).
- `ensure` — `no-op` آمن يُنشئ القاعدة إن غابت فقط (يُشغَّل في `releaseCommand`).

### ملاحظات الإنتاج

- **عمّال gunicorn**: `2 workers × 4 threads` كافية لـ SQLite+WAL (المخطط يضبط `journal_mode=WAL` و`busy_timeout=30000` لكل اتصال). لمزيد من الزخم ارفع العدد أو انقل إلى PostgreSQL لاحقًا.
- **النسخ الاحتياطي**: `python scripts/backup.py backup` على القرص المُثبَّت (جدولة خارجية مُوصى بها).
- **الواجهة الأمامية**: تُقدَّم من `frontend/` على نفس الأصل — لا تعطيل CORS للاستخدام العادي (CORS لطلبات النطاقات الأخرى).
...
10. **نظام الإعلانات (Roadmap Phase 6 — اكتملت)**: مكتمل (قرار D-027): فتحات
    ...
    استهداف v1: فتحة + تواريخ فقط (§5)؛ الاستهداف الفئوي (v2) اكتمل
    بالمرحلة 19 (D-037)؛ والربط بالفوترة (فواتير/ميزانيات) لاحقًا.
    **بقي من Roadmap Phase 6**: هجرة
...
14. **تحسينات البحث العربي (المرحلة 14 — اكتمل)**: مكتمل (قرار D-032):
    ...
    الموجَّه. **الباقي**: إشعارات push/email، جاهزية multi-tenant.
15. **العمليات الإدارية الجماعية (المرحلة 15 — اكتمل)**: مكتمل (قرار
    D-033): حزم عمليات بنموذج استجابة موحّد
    {action,total,succeeded,failed,results} ونجاح جزئي موثّق في معاملة
    واحدة: قبول/رفض جماعي للتحقق (`/api/admin/verification/bulk`)،
    معالجة جماعية للبلاغات (`/moderation/bulk`)، حذف/تحديث جماعي للنصوص
    والمواد (`/texts/bulk-delete`، `/texts/bulk-update`،
    `/articles/bulk-delete`)، إيقاف/استئناف/إنهاء جماعي للحملات
    (`/ads/campaigns/bulk-status`)، وحذف جماعي لقوالب السوق
    (`/marketplace/templates/bulk-delete`) مع سقف 200 معرّف وتدقيق
...
16. **تسليم الإشعارات الخارجية (المرحلة 16 — اكتمل)**: مكتمل (قرار
    ...
    **الباقي**: لا شيء — اكتملت بنود الخريطة بالمرحلة 17.
17. **جاهزية multi-tenant (المرحلة 17 — اكتمل)**: مكتمل (قرار D-035):
    عزل الهوية فقط (لا عزل بيانات الوحدات — مؤجَّل لمرحلة multi-tenancy
    فعلية): جدول `tenants` بمستأجر افتراضي مبذور تلقائيًا
    (`NIBRAS_DEFAULT_TENANT_SLUG`، افتراضيًا `nibras`) وربط كل مستخدم
    بمستأجره عبر `users.tenant_id` بترحيل آمن للموجودين عند الإقلاع،
    وclaim `tenant_id` في JWT. عند `NIBRAS_MULTI_TENANT=1` يُلزَم رأس
    `X-Tenant-Id` في الطلبات المصادَق عليها (غياب/جهل/تعارض/تعليق → 403)
    ويُربط التسجيل بمستأجره؛ وفي الوضع الافتراضي يُتجاهل الرأس تمامًا.
    لوحة إدارية دنيا: `GET/POST /api/admin/tenants` مع تدقيق لكل إنشاء،
    وفحص المستأجرين في `/api/ready`. البوابة بعدها: 486 اختبارًا ناجحًا
    (452+34) + فحص حي على نسخة من nibras.db الحقيقية أثبت ترحيل القاعدة
    القديمة وسلوك الرأس في الوضع المفعّل.
18. **عزل بيانات الوحدات multi-tenant (المرحلة 18 — اكتمل)**: مكتمل
    (قرار D-036): العزل الكامل بثلاث طبقات — عمود `tenant_id` على 17
    جدولًا معزولًا (المكتبة/المجتمع/المحترفون/السوق/الإعلانات/الاجتهاد)
    بفهارس `idx_*_tenant`، سياق `app/tenant_scope.py` (ContextVar بلا
    Flask) بمساعدة `tenant_eq` / `insert_tenant_id`، وفرض شامل في
    `before_request` عند `NIBRAS_MULTI_TENANT=1` يلزم `X-Tenant-Id`
    (403 للغياب/الجهل/التعليق، واستثناء health/ready/CORS). كل قراءة
    قائمة/تفصيل/تحديث في الوحدات المعزولة مُقيَّدة بالمستأجر (العودة
    404 لصف مستأجر آخر، لا تسريب وجود)، والحسّابات الفرعية والبحث FTS
    وأوزان JOIN تُحتسب ضمن نطاق المستأجر فقط، وملخص التحليلات محصور
    بمستأجر الطلب؛ ترحيل قديم (idempotent) يضيف الأعمدة والفهارس ويلحق
    الصفوف بالمستأجر الافتراضي. البوابة بعدها: 532 اختبارًا ناجحًا
    (486+46، منها 13 لعزل multi-tenant) + ruff نظيف.
19. **الاستهداف الفئوي للحملات الإعلانية (المرحلة 19 — اكتمل)**: مكتمل
    (قرار D-037): يُرفع استهداف v1 (فتحة + تواريخ فقط) إلى استهداف
    فئوي — عمودان اختياريان على `ad_campaigns`
    (`target_category_type` في library|marketplace|jurisprudence و
    `target_category_id` يرجع لفئة في جدول نوعها) بمهاجرة `_ensure_column`
    للقواعد القائمة؛ `serve(slot, category_type, category_id)` تُفضّل
    الحملة المستهدفة للفئة المطابقة ثم ترجع إلى العامة، ودون سياق فئة
    لا تُعرض المستهدفة إلا العامة؛ تحقق على الإنشاء/التحديث أن النوع ضمن
    القائمة والفئة موجودة في جدولها ضمن مستأجر الطلب، وقيمة فارغة
...
20. **وحدة القانون المقارن (المرحلة 20 — اكتمل)**: مكتمل (قرار D-038):
    ...
    البوابة بعدها: 580 اختبارًا ناجحًا + ruff نظيف.
